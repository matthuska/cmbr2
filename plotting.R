...
#' @export
#' 
plotProfile <- function( coverage, x.bins=NA, groupings=NA, grouping.order=NA, color=NA, method="median", scale.=T, mark.quartiles=T, markings=NULL, ... ) {
	...
	for ( i in 1:k ) {
		...
		# start drawing
		plot(1, type="n", main=k.names[i], ylim=ylim, xlim=c( min(x.values) ,max(x.values)), ... )
